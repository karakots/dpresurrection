...
namespace MarketSimUtilities
{
	...
	{
		public const int AllID = ModelDb.AllID;
		...
		/// <summary>
		/// set width uses the coumn width as the relative size for cols
		/// </summary>
		...
		static public void SetWidth(DataGrid datagrid, DataView dataview)
		{
			...
			foreach(DataGridTableStyle table in datagrid.TableStyles)
			{
				...
				if(diff > 0 && numCols > 0)      // jimj 30Apr07 - don't ever shrink columns (I added "diff>0 &&" )
				{
					// add to each col
					if (numCols != 0)
					{
        ...
        /// <summary>
        /// computes the width based on data in grid
        /// can be expensive for very large tables - should only be called if data changes
        /// </summary>
        /// <param name="datagrid"></param>
        /// <param name="dataview"></param>
        static public void ComputeWidth(DataGrid datagrid, DataView dataview)
        {
            Graphics g = Graphics.FromHwnd(datagrid.Handle);
            StringFormat sf = new StringFormat(StringFormat.GenericTypographic);
		...
		static public DataGridTableStyle DisabledTableStyle()
		{
			DataGridTableStyle tableStyle = new DataGridTableStyle();
			tableStyle.AlternatingBackColor = Color.LightGray;
			tableStyle.SelectionBackColor = Color.LightGray;
			tableStyle.BackColor = Color.LightGray;

			tableStyle.HeaderBackColor = Color.LightGray;
			tableStyle.HeaderForeColor = Color.DarkGray;
		...
		public MrktSimControl(ModelDb db)
		{
			// This call is required by the Windows.Forms Form Designer.
			InitializeComponent();

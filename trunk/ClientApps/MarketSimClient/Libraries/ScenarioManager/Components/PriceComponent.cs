...
namespace DecisionPower.MarketSim.ScenarioManagerLibrary.Components
{
    public class PriceComponent : Component
    {
        ...
        /// </summary>
        /// <returns></returns>
        public PriceComponentData[] GetData() {
        ...
        /// <returns></returns>
        public PriceComponentData[] GetData( DateTime start, DateTime end, Enums.DataIntervalCheckType dataCheckType ){
        ...
        /// <returns></returns>
        public PriceComponentData[] GetData( DateTime start, DateTime end, Enums.DataIntervalCheckType dataCheckType, Channel channel ){

            string dataQuery = DataQuery( start, end, dataCheckType, channel );

            MrktSimDBSchema.product_channelRow[] rows = (MrktSimDBSchema.product_channelRow[])this.dataTable.Select( dataQuery );

            int rowCount = rows.Length;
            PriceComponentData[] data = new PriceComponentData[ rowCount ];
            for( int i = 0; i < rowCount; i++ ) {

                data[ i ] = new PriceComponentData( this.model, rows[ i ] );
            }
            return data;
        }
        ...
        /// <returns></returns>
        public int AddData( PriceComponentData[] newComponentData ){

            for( int i = 0; i < newComponentData.Length; i++ ) {
                MrktSimDBSchema.product_channelRow newRow = (MrktSimDBSchema.product_channelRow)this.dataTable.NewRow();

                newRow.start_date = newComponentData[ i ].StartDate;
                newRow.end_date = newComponentData[ i ].EndDate;
                newRow.price = newComponentData[ i ].Price;
                newRow.percent_SKU_in_dist = newComponentData[ i ].PercentDistribution;

                newRow.price_type = newComponentData[i].PriceType;
        ...
        /// <returns></returns>
        public new PriceComponent Copy( string newName, string newDescription ) {
            MrktSimDBSchema.market_planRow copyRow = model.ModelDb.CopyPlan( this.componentRow );
            PriceComponent copyComponent = new PriceComponent( this.model, copyRow );
        ...
        /// <summary>
        /// Constructs a new PriceComponent object.  For framework usage only.
        /// </summary>
        ...
        public PriceComponent( Model model, MrktSimDBSchema.market_planRow componentRow ) : base( model, componentRow ) {

            this.dataTable = theDb.Data.product_channel;
        }
    }
}

...
/////////////////////////////////////////////////////////////////////////////
// 
IMPLEMENT_DYNAMIC(ChannelRecordset, CRecordset)

ChannelRecordset::ChannelRecordset(CDatabase* pdb)
	: CRecordset(pdb)
{
	//{{AFX_FIELD_INIT(ChannelRecordset)
	m_ChannelID = 1;
	m_ChannelName = _T("");
	m_nFields = 2;
	//}}AFX_FIELD_INIT
...
CString ChannelRecordset::GetDefaultConnect()
{
...
CString ChannelRecordset::GetDefaultSQL()
{
	CString tmp("SELECT channel_id, channel_name FROM channel ");
	tmp += ModelRecordset::ModelQuery;

	return tmp;
}

void ChannelRecordset::DoFieldExchange(CFieldExchange* pFX)
{
	...
	//{{AFX_FIELD_MAP(ChannelRecordset)
	pFX->SetFieldType(CFieldExchange::outputColumn);
	RFX_Long(pFX, _T("[channel_id]"), m_ChannelID);
	RFX_Text(pFX, _T("[channel_name]"), m_ChannelName);
	//}}AFX_FIELD_MAP
...
/////////////////////////////////////////////////////////////////////////////
#ifdef _DEBUG
void ChannelRecordset::AssertValid() const
{
	CRecordset::AssertValid();
}

void ChannelRecordset::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG

int ChannelRecordset::Open(unsigned int nOpenType, LPCTSTR lpszSql, DWORD dwOptions) 
{

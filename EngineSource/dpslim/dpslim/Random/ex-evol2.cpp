...
//-----------------------------------------------------------------------------
void SplitIntoGenotypes(int32 * genotypes, int32 * genes, StochasticLib1 & stoc) {
  ...
  int32 g;            // total number of genes
  int32 na_;          // number of individuals with gene a on first place
...
//-----------------------------------------------------------------------------
int BreedingSuccess(double * ChildGenotypes, int32 * FathersGenotypes, 
  int32 * MothersGenotypes, double * Selection, int Mul) {
...
int main() {
  ...
  //---------------------------------------------------------------------------
  int32 PopulationSize = 100;          // initial population size
  double MutationRate = 1.E-5;         // mutation rate
  double GrowthRate = 1.10;            // unrestrained growth rate
  int32 CarryingCapacity = 100000;     // maximum number of individuals in habitat
  ...
  int32 Generations = 4000;            // number of generations to simulate
  ...
  // other variables
  int32 ParentGenePool[2];             // number of genes a and b
  int32 ChildGenePool[2];              // gene pool of child generation
  int32 GenerationNumber;              // generation number
  int32 Mutations;                     // forward - backwards mutations
  int32 Children;                      // number of individuals in child population
  int32 MaxPopulation;                 // maximum population size
  int32 GenotypesFemales[3];           // genotypes (a,a), (a,b), and (b,b)
  int32 GenotypesMales[3];             // same, males only
  int32 GenotypesChildren[3];          // same, children
  ...
  int32 OutputPeriod;                  // period between outputs
  ...
  for (GenerationNumber = 1; GenerationNumber <= Generations; GenerationNumber++) {
    ...
    MaxPopulation = (int32)sto.Normal(CarryingCapacity, CapacityDeviation); 

...
*/   
  ...
  if (n != bino_n_last || p != bino_p_last) {
    ...
    bino_bound = (int32)(rc + 11.0*(sqrt(rc) + 1.0));
...
*/  
  ...
  if (n != bino_n_last || p != bino_p_last) {               // set-up
    ...
    mode = (int32) nu;
    Bino_k2 = (int32) ceil(nu - 0.5 - W);
    Bino_k4 = (int32)     (nu - 0.5 + W);
  ...
  for (;;) {
    ...
    if ((U = Random() * Bino_p6) < Bino_p2) {        // centre left
      ...
      if ((V = U - Bino_p1) < 0.) return(Bino_k2 + (int32)(U/Bino_f2));
      // immediate acceptance region R1 = [k1, k2)*[0, f1),  X = k1, ... k2-1
      if ((W = V / Bino_dl) < Bino_f1) return(Bino_k1 + (int32)(V/Bino_f1));

      // computation of candidate X < k2, and its counterpart Y > k2
      // either squeeze-acceptance of X or acceptance-rejection of Y
      Dk = (int32)(Bino_dl * Random()) + 1;
      ...
      if ((V = U - Bino_p3) < 0.) return(Bino_k4 - (int32)((U - Bino_p2)/Bino_f4));
      // immediate acceptance region R4 = [k4+1, k5+1)*[0, f5)
      if ((W = V / Bino_dr) < Bino_f5) return(Bino_k5 - (int32)(V/Bino_f5));

      // computation of candidate X > k4, and its counterpart Y < k4
      // either squeeze-acceptance of X or acceptance-rejection of Y
      Dk = (int32)(Bino_dr * Random()) + 1;
...
*/
  ...
  if (L != pois_L_last) {  // set up
    pois_L_last = L;
    pois_bound = (int32)floor(L+0.5 + 7.0 * (sqrt(L+L+1.) + 1.5));// safety-bound
...
*/
  ...
  if (L != pois_L_last) { // set-up
    ...
    // delimit the centre region of h(x)
    mode = (int32) L;
    Pois_k2 = (int32) ceil(L - 0.5 - Ds);
    Pois_k4 = (int32)     (L - 0.5 + Ds);
  ...
  for (;;) {
    ...
    if ((U = Random() * Pois_p6) < Pois_p2) {                    // centre left
      ...
      // immediate acceptance region R2 = [k2, mode) *[0, f2),  X = k2, ... mode -1
      if ((V = U - Pois_p1) < 0.0)  return(Pois_k2 + (int32)(U/Pois_f2));
      // immediate acceptance region R1 = [k1, k2)*[0, f1),  X = k1, ... k2-1
      if ((W = V / Pois_dl) < Pois_f1 )  return(Pois_k1 + (int32)(V/Pois_f1));

      // computation of candidate X < k2, and its counterpart Y > k2
      // either squeeze-acceptance of X or acceptance-rejection of Y
      Dk = (int32)(Pois_dl * Random()) + 1;
      ...
      //  immediate acceptance region R3 = [mode, k4+1)*[0, f4), X = mode, ... k4
      if ((V = U - Pois_p3) < 0.)  return(Pois_k4 - (int32)((U - Pois_p2)/Pois_f4));
      // immediate acceptance region R4 = [k4+1, k5+1)*[0, f5)
      if ((W = V / Pois_dr) < Pois_f5)  return(Pois_k5 - (int32)(V/Pois_f5));

      // computation of candidate X > k4, and its counterpart Y < k4
      // either squeeze-acceptance of X or acceptance-rejection of Y
      Dk = (int32)(Pois_dr * Random()) + 1L;
...
*/
  ...
  if (N != hyp_N_last || m != hyp_m_last || n != hyp_n_last) { 
    ...
    mode  = (int32)modef;
    Hyp_k2 = (int32)ceil(modef - 0.5 - U);  
    if (Hyp_k2 >= mode) Hyp_k2 = mode - 1;
    Hyp_k4 = (int32)(modef - 0.5 + U);
    ...
    while (1) {
      ...
      if ((U = Random() * Hyp_p6) < Hyp_p2) {                  // centre left
        ...
        // immediate acceptance region R2 = [k2, mode) *[0, f2),  X = k2, ... mode -1
        if ((W = U - Hyp_p1) < 0.)  return(Hyp_k2 + (int32)(U/Hyp_f2));
        // immediate acceptance region R1 = [k1, k2)*[0, f1),  X = k1, ... k2-1
        if ((Y = W / Hyp_dl) < Hyp_f1)  return(Hyp_k1 + (int32)(W/Hyp_f1));
        ...
        // immediate acceptance region R3 = [mode, k4+1)*[0, f4), X = mode, ... k4
        if ((W = U - Hyp_p3) < 0.)  return(Hyp_k4 - (int32)((U - Hyp_p2)/Hyp_f4));
        
        // immediate acceptance region R4 = [k4+1, k5+1)*[0, f5)
        if ((Y = W / Hyp_dr) < Hyp_f5)  return(Hyp_k5 - (int32)(W/Hyp_f5));

        // computation of candidate X > k4, and its reflected counterpart V < k4
        // either squeeze-acceptance of X or acceptance-rejection of V
        Dk = (int32)(Hyp_dr * Random()) + 1;

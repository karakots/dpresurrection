...
int32 StochasticLib3::WalleniusNCHypInversion (int32 n, int32 m, int32 N, double odds) {
   ...
   int32 wall_x1, x2;                   // search values
   int32 xmin, xmax;                    // x limits
   ...
   wall_x1 = (int32)(wnch1.mean());          // start at floor and ceiling of mean
   x2 = wall_x1 + 1;
   xmin = m+n-N; if (xmin<0) xmin = 0;  // calculate limits
   xmax = n;     if (xmax>m) xmax = m;
...
int32 StochasticLib3::FishersNCHypInversion (int32 n, int32 m, int32 N, double odds) {
   /* 
...
int32 StochasticLib3::FishersNCHypRatioOfUnifoms (int32 n, int32 m, int32 N, double odds) {
   /* 
   Subfunction for FishersNCHyp distribution. 

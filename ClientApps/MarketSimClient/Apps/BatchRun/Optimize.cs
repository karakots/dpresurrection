...
namespace BatchRun
{
	...
	public class Optimize
	{
		...
		private const double stepScale = 0.1;			
		private const int numEvals = 2;
		...
		double curVal = 0.0;
		double stepSize = stepScale/(2*numEvals);

		double curMinVal =  10000000.0;
		double curMaxVal = -10000000.0;

		// 10 % tolerance on range
		// this gets multiplied by actual range
		...
		public bool Run(int maxIters)
		{
			...
			// we have a 1% tolerance on range
			tolerance = tolerance * (curMaxVal - curMinVal);
		...
		private bool computeStep(out bool cancel)
		{
			...
			{
				double step = stepSize * (max[ii] - min[ii]);
				...
				for(int jj = 0; jj < numEvals; ++jj)
				{
					...
					if (Evaluate != null)
						y[1 + numEvals + jj] = Evaluate(out cancel);

					// step back
					var[ii] = x[0];
				}

				grad[ii] = computeSlope(x,y, out acc[ii]);
			}
			...
			for (int ii = 0; ii < var.Length; ++ii)
			{
				...
				if (Math.Abs(grad[ii]) > smallFloat && acc[ii] > smallFloat)
				{
					nextStep = var[ii] - grad[ii]/acc[ii];
		...
		private double computeSlope(double[] x, double[] y, out double acc)
		{
			acc = 0.0;

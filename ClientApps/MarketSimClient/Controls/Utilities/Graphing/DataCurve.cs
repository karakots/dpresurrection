	...
	public class DataCurve
	{
		...
		public TimeSpan Span
		{
			get
			{
				...
				if(x.Count == 1 || x.Count == 0 || x[1] <= x[0])
				{
					return span;
				}
				else
		...
		public double Eval(DateTime day)
		{
			...
			if(val > x[x.Count - 1] || day.CompareTo(earliest) < 0)
			{
				return 0;
			}
			...
			if( val == x[x.Count - 1] )
			{
				return y[y.Count - 1]/((double)GetSpan(x.Count - 1).Days);
			}
			...
			while (x[index] < val && index < x.Count - 1)
			{
				index++;
			}
		...
		{
            x = new List<double>( numVals );
            y = new List<double>( numVals );
			index = 0;
		}
		...
		public DataCurve Average(DateTime start, DateTime end, TimeSpan numDays, HowToSum sumType)
		{
            if( sumType == HowToSum.Constant) {
                Constant = true;
                return this;
            }
			...
			// aveLength*numItems < x.Count + 1 (see where we perform assignment in loop)

			// if Count is 21 and numItems is 7 (one week)
			// then aveLength is 3
            ...
            if (aveLength == 1)
            {
                // special case we want sum the entire curve up
                double ave = 0.0;
                for (DateTime curDay = start; curDay < end; curDay = curDay.AddDays(1))
                {
                    if (sumType == HowToSum.Max)
                    {
                        ave = Math.Max(ave, Eval(curDay));
                    }
                    else
                    {
                        ave += Eval(curDay);

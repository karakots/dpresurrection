...
//		}
		...
		public bool EnabledGrid
		{
			set
			{
				...
				if (Enabled)
				{
					...
					this.iTableStyle.AlternatingBackColor = Color.LightBlue;
					this.iTableStyle.SelectionBackColor = Color.DarkSeaGreen;
					this.iTableStyle.BackColor = DataGridTableStyle.DefaultTableStyle.BackColor;
					this.iTableStyle.ForeColor = DataGridTableStyle.DefaultTableStyle.ForeColor;

					this.iTableStyle.HeaderBackColor = DataGridTableStyle.DefaultTableStyle.HeaderBackColor;
					this.iTableStyle.HeaderForeColor = DataGridTableStyle.DefaultTableStyle.HeaderForeColor;
				}
				else
				{
					

					this.iTableStyle.AlternatingBackColor = Color.LightGray;
					this.iTableStyle.SelectionBackColor = Color.LightGray;
					this.iTableStyle.BackColor = Color.LightGray;
					this.iTableStyle.ForeColor = Color.DarkGray;

					this.iTableStyle.HeaderBackColor = Color.LightGray;
					this.iTableStyle.HeaderForeColor = Color.DarkGray;
				}
		...
		public  ArrayList GetSelected()
		{
            ...
            // sometimes the user just wants the current
            if (rows.Count == 0 && this.CurrentRow != null)
            {
		...
		public string RowFilter
		{
			set
			{
				...
				baseFilter = value;
                ClearSorting();     //JimJ - fixing bug 000044
			}
		...
		private System.Windows.Forms.MenuItem filterExpression;
		...
		/// <summary>
		/// Set read Only after setting table style
		/// </summary>
		public bool ReadOnly
		{
			set
			{
				...
				foreach(DataGridTextBoxColumn col in this.iTableStyle.GridColumnStyles)
				{
					col.ReadOnly = true;
				}
			}
			...
			public DataMatrix DataMatrix
		{
			set
			{
				...
				value.DataMatrixColumnChanged +=new MarketSimUtilities.DataMatrix.DataMatrixColumnChangedHandler(dataMatrix_ColumnChanged);
        ...
        private string sortSettingWhenSuspended = null;            //JimJ - fixing bug #000021
		...
		public  bool Suspend
		{
			set
			{
                ...
                if( value ) {
                    sortSettingWhenSuspended = dataView.Sort;         //JimJ - fixing bug #000021
                    dataView.Sort = null;
                    dataView.RowFilter = null;
                }
                else {
                    dataView.Sort = sortSettingWhenSuspended;        //JimJ - fixing bug #000021
		...
		public void Reset()
		{
			...
			MrktSimControl.ComputeWidth(this.dataGrid, this.dataView);
		...
		{
			return AddCheckBoxColumn(colName, 
				MrktSimControl.MrktSimColumnName(iTableStyle.MappingName, colName), 
				trueVal, falseVal, false);
		}

		public DataGridBoolColumn AddCheckBoxColumn(string colName, string mappingName, string trueVal, string falseVal, bool readOnly)
		{
			// product attribute relevance
			DataGridBoolColumn col = new DataGridBoolColumn();
		...
		public MrktSimGrid()
		{
			...
			this.filterExpression.Visible = false;

			if (MrktSimControl.MrktSimMessage("devl_version") == "true")
			{
				this.filterExpression.Visible = true;
			}	
		}
		...
		private void InitializeComponent()
		{
			...
			this.filterExpression = new System.Windows.Forms.MenuItem();
			...
			this.filterItem.Click += new System.EventHandler(this.filterItem_Click);
			this.filterItem.Select += new System.EventHandler(this.filterItem_Click);
			// 
			// includeFilteritem
			// 
			...
			// filterExpression
			// 
			this.filterExpression.Index = 2;
			this.filterExpression.Text = "expression";
			this.filterExpression.Click += new System.EventHandler(this.filterExpression_Click);
			// 
			// MrktSimGrid
			// 
        ...
        private void ClearSorting() {
            this.sortSettingWhenSuspended = null;
		...
		private void parametrizeRow(DataRow row)
		{
			...
			MrktSimDBSchema.model_parameterRow parm = theDb.CreateModelParameter(theDb.ModelID, row, colName, RowID);
		...
		private void changePrecision_Click(object sender, System.EventArgs e)
		{
			...
			InputDouble dlg = new MarketSimUtilities.InputDouble();
			dlg.Max = 14;
			dlg.Min = 0;
			dlg.Value = current;
		...
		private void filterItem_Click(object sender, System.EventArgs e)
		{
			if (currentColStyle == null)
			{
				this.filterExpression.Enabled = false;
				this.includeFilteritem.Enabled = false;
				this.excludeFilterItem.Enabled = false;
				return;
			}
			...
			{
				this.filterExpression.Enabled = true;
			}
			else
			{
				this.filterExpression.Enabled = false;
		...
		private void filterOnExpression(string expression)
		{
			
			if (currentColStyle == null)
				return;

			string query = currentColStyle.MappingName + " " + expression;
			...
			catch(Exception)
			{
				...
				MessageBox.Show("Unable to process query: " + query );
		...
		private void filterSelection(string equals)
		{
			...
			{
				MessageBox.Show("Cannot filter on this row");
				return;
			}
			...
			string expression = equals + " '" + row[currentColStyle.MappingName].ToString() +"'";

			filterOnExpression(expression);	
		}
		...
		private void filterExpression_Click(object sender, System.EventArgs e)
		{
			...
			string expression = "LIKE '" + dlg.InputText + "'";

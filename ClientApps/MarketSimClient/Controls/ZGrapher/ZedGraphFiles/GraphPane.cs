...
namespace ZedGraph
{
	...
	/// <version> $Revision: 1.3 $ $Date: 2006/06/22 21:05:17 $ </version>
	...
	#region Rendering Methods
        ...
        {			
			...
			if ( showGraf )
			{
				...
				// Draw the Pane Title
				DrawTitle( g, this, fontScaleFactor );
			...
			if ( showGraf )
			{
				...
				// Draw the Legend
				this.legend.Draw( g, this, fontScaleFactor, hStack, legendWidth,
							legendHeight );
									...
									out int hStack, out float legendWidth, out float legendHeight )
		{
			...
			// changed so we leave fonts alone if not changing their scale
			// SSN 8/1/2005
			...
			float charHeight = this.FontSpec.GetHeight( fontScaleFactor );
			...
			if ( this.isShowTitle )
			{
				SizeF titleSize = this.FontSpec.BoundingBox( g, this.title, fontScaleFactor );
			...
			this.legend.CalcRect( g, this, fontScaleFactor, ref tmpRect,
		...
		public double CalcScaleFactor()
		{
			...
			// Changed this so we calculate scale only for fonts
			// SSN 8/1/2005
//			if ( !this.isFontsScaled )
//				return 1.0;
